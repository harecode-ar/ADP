...
export const getTooltipFromAcp = (
  ...
  if (acp <= 0) {
    return `Se ahorra ${calc(acp)}% del tiempo estipulado para ${
      type === 'project' ? 'el proyecto' : 'la etapa'
    }`
  }
  if (percentageAlertMargin >= acp) {
    return `Se excede ${calc(acp)}% del tiempo estipulado para ${
      type === 'project' ? 'el proyecto' : 'la etapa'
    }`
  }
...
export const getTooltipFromPacp = (
  ...
  if (pacp <= 0 && pacp > -percentageAlertMargin) {
    return `En promedio se esta excediendo un ${calc(pacp)}% del tiempo estipulado para sus ${
      type === 'project' ? 'proyectos' : 'etapas'
    }`
  }
  if (pacp > 0) {
    return `En promedio se esta excediendo un ${calc(pacp)}% del tiempo estipulado para sus ${

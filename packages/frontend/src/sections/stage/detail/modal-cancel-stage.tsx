...
export default function ModalCancelStage(props: TProps) {
  ...
  const onAccept = async () => {
    if (loading) return
    try {
      if (!stage) throw new Error('No se ha encontrado la etapa.')
      ...
      enqueueSnackbar('La etapa se ha cancelado corractamente.', { variant: 'success' })
      modal.onFalse()
      refetch()
    } catch (error) {
    ...
    >
      <Box sx={DEFAULT_STYLE_MODAL}>
        ...
        <Typography id="modal-description" sx={{ mt: 2 }}>
          ¿Está seguro que desea cancelar la etapa?

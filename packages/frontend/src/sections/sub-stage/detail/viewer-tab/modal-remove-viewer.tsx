...
import { REMOVE_STAGE_VIEWER } from 'src/graphql/mutations'
...
export default function ModalRemoveViewer(props: TProps) {
  const { modal, refetch, userId, stageId } = props
  const [removeStageViewer, { loading }] = useMutation(REMOVE_STAGE_VIEWER)
  const { enqueueSnackbar } = useSnackbar()

  const onAccept = async () => {
    if (loading) return
    try {
      const { errors } = await removeStageViewer({
        variables: {
          userId,
          stageId,
        },
